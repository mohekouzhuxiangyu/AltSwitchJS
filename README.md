...
4. **改快捷键**：点「修改…」，直接按下新组合（例如 `Ctrl + Alt + F9`）
5. **专注模式**：右上角开关，开启后切换时只保留当前窗口
6. **隐藏**：关闭窗口 = 最小化（快捷键仍生效）；Windows 托盘 / macOS 程序坞点击恢复
...
```json
{
  "mods": 1,                    // 1=Alt 2=Ctrl 4=Shift 8=Win/Cmd
  "key": "`",                   // 快捷键主键
  "selected": ["pid|进程|标题"], // 勾选的窗口
  "focusMode": false            // 专注模式开关
}
```
