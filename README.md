...
- **打开主窗口快捷键（可自定义）**：默认**双击 `Cmd`**（Windows 双击 `Win`），快速按两下即在打开/隐藏主窗口间切换（任意应用内生效）；也支持在「修改…」里设为任意组合键
...
5. **唤出主窗口**：任意时刻**双击 `Cmd`**（Windows 双击 `Win`）在打开/隐藏主窗口间切换，即使窗口已隐藏/最小化；也可自定义为任意组合键
...
```json
{
  ...
  "key": "`",                   // 切换快捷键主键
  "winMode": "double",          // 打开主窗口模式：double=双击修饰键 / chord=组合键
  "winDoubleMods": 8,           // 双击的修饰键（8 = Cmd/Win，也可 Alt/Ctrl/Shift）
  "winMods": 13,                // 组合键模式下的修饰键（13 = Alt+Shift+Cmd）
  "winKey": "A",                // 组合键模式下的主键
